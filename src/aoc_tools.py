...
def all_pairs(elements: Iterable[T]) -> Generator[Tuple[T, T], None, None]:
    elements_list = list(elements)
    for i in range(len(elements_list)):
        for j in range(i + 1, len(elements_list)):
            yield (elements_list[i], elements_list[j])

def all_tuples(elements: Iterable[T]) -> Generator[Tuple[T, T], None, None]:
    elements_list = list(elements)
    for i in range(len(elements_list)):
...
def rolling_sum(elements: Iterable[T], start: T = None) -> List[T]:
    rsum = []
    elements_iter = iter(elements)
